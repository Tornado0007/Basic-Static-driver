...
{       
	int ret =0;
	 ...
	 printk("Start_code :%lu \nEnd_code :%lu,Start_data : %lu End_data :%lu Arg_start :%lu\n Arg_end : %lu\n Env_start : %lu\n Env_end :%lu\n start_brk(heap) :%lu\nbrk :%lu\n",current->mm->start_code,current->mm->end_code,current->mm->start_data,current->mm->end_data,current->mm->arg_start,current->mm->arg_end,current->mm->env_start,current->mm->env_end,current->mm->start_brk,current->mm->brk);
