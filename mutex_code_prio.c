...
int thread_function2(void *pv)
{
	int i=0;
       ...
       current->rt_priority=40;
   	//current->static_prio=60;
	//current->normal_prio=60;
 	//current->prio=60;
	
        down(&mysema);
 
        printk("T2 Priority :%d\nStatic_priority :%d\nNormal Priority :%d\nrt Priority %d\n",current->prio,current->static_prio,current->normal_prio,current->rt_priority);
        for(i=0;i<50;i++){
        etx_global_variable++;
...
static int __init etx_driver_init(void)
{
        ...
        if((dev_class = class_create(THIS_MODULE,"Prio_class")) == NULL){
            pr_info("Cannot create the struct class\n");
            goto r_class;
        }
 
        /*Creating device*/
        if((device_create(dev_class,NULL,dev,NULL,"Prio_device")) == NULL){
        ...
        /* Creating Thread 1 */
        etx_thread1 = kthread_run(thread_function1,NULL,"T1");
        if(etx_thread1) {
         ...
         /* Creating Thread 2 */
        etx_thread2 = kthread_run(thread_function2,NULL,"T2");
        if(etx_thread2) {
