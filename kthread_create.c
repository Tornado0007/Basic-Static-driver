...
static struct task_struct *etx_thread1,*etx_thread2;
...
{
    int i=0;
    printk("prcess : %d  thrd : %d and Add :%p",current->pid,current->tgid,(void *)pv);
    while(!kthread_should_stop()) {
	printk("process : %d  thread : %d",current->pid,current->tgid);
...
static int __init etx_driver_init(void)
{
	...
	p=0x40;
	printk("p: %p",p);
        etx_thread1 = kthread_create(thread_function,p,"T1");
        if(etx_thread1) {
	...
	p=0x50;
	printk("p: %p",p);
	etx_thread2 = kthread_create(thread_function,p,"T2");
        if(etx_thread2) {
