...
{
	printk(KERN_INFO"Request number : %d and  device id :\n",irq);
	i++;
	return IRQ_HANDLED;
}
//***************************Interrupts End***************************************
...
int i=0;
	...
	if((list = (struct list*)kmalloc(nr_irqs+1,GFP_KERNEL))==0)
	{
		printk(KERN_INFO "Cannot allocate memory in kernel\n");
		return -1;
	}
        i=0;
	for_each_irq_desc(list[i].irq,list[i].desc)
	++i;
	
	for(i=0;i<nr_irqs;i++)
	printk("Irq nu %d name :%s,node : %u status :%d",list[i].irq,list[i].desc->name,list[i].desc->irq_count,list[i].desc->status_use_accessors);
	//Copy the data from the kernel space to the user-space
    	//ret = copy_to_user(buf, , MEM_SIZE);
	if(ret > 0)
	{
